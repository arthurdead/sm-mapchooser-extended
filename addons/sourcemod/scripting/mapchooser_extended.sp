...
ArrayList g_MapNameUFilter;
ArrayList g_MapNameUReplace;
...
public void OnPluginStart()
{
	...
	g_MapNameUFilter = new ArrayList(arraySize);
	g_MapNameUReplace = new ArrayList(arraySize);
						   ...
						   NativeVote voteNative=null)
{
	...
	else
	{
		...
		{
			LogAction(-1, -1, "change map in 5.0");
			DataPack data;
		...
		GetMapName(displayName, mapName, sizeof(mapName));

		if (voteNative != null)
		{
			voteNative.DisplayPass(mapName);
		}
		
		PrintToChatAll("[MCE] %t", "Nextmap Voting Finished", mapName, RoundToFloor(float(item0_votes)/float(num_votes)*100), num_votes);
...
public int Handler_MapVoteMenu(Menu menu, MenuAction action, int param1, int param2)
{
	switch (action)
	{
		...
		case MenuAction_VoteCancel:
		{
			...
			{
				ExtendMap();
				CPrintToChatAll("[MCE] %t", "No Vote Extend", g_Cvar_ExtendTimeStep.IntValue);
...
NominateResult InternalNominateMap(char[] map, bool force, int owner)
{
	...
	if (!g_Cvar_NominateAll.BoolValue && g_NominateCount >= GetVoteSize() && !force)
	{
...
{
	g_MapNames.Clear();
	g_MapNameUFilter.Clear();
	g_MapNameUReplace.Clear();
	g_tempMapFile = 0;
	char mapNamesPath[PLATFORM_MAX_PATH];
	SMCParser SMC = new SMCParser(); 
	SMC.OnEnterSection = NewSection;
	SMC.OnKeyValue = KeyValue;
	SMC.OnLeaveSection = EndSection;
	SMC.ParseFile(mapNamesPath);
...
public SMCResult KeyValue(Handle smc, const char[] key, const char[] value, bool key_quotes, bool value_quotes)
{
	if (g_tempMapFile == 0)
	{
		if (!g_MapNames.SetString(key, value, false))
		{
...
public int Native_CanNominate(Handle plugin, int numParams)
{
	...
	if (!g_Cvar_NominateAll.BoolValue && g_NominateCount >= GetVoteSize())
	{
...
public int Native_GetMapName(Handle plugin, int numParams)
{
	...
	if (g_MapNames.Size > 0 && g_MapNames.GetString(map, mapName, sizeof(mapName)))
	{
		SetNativeString(2, mapName, len);
		return 1;
	}

	int arraySize = g_MapNameUFilter.Length;
	if (arraySize > 0)
	{
		char tempMapName[PLATFORM_MAX_PATH];
		strcopy(tempMapName, sizeof(tempMapName), map);
		...
		{
			g_MapNameUFilter.GetString(i, tempFilter, sizeof(tempFilter));

			if (StrContains(tempMapName, tempFilter, false) >= 0)
			{
				g_MapNameUReplace.GetString(i, tempReplacment, sizeof(tempReplacment));
				ReplaceString(tempMapName, sizeof(tempMapName), tempFilter, tempReplacment, false);
			}
		}

		strcopy(mapName, sizeof(mapName), tempMapName);
		SetNativeString(2, mapName, len);
		return 1;
	}

	strcopy(mapName, sizeof(mapName), map);
	SetNativeString(2, mapName, len);
	return 0;
}
